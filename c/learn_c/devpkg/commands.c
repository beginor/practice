...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
	...
	if (apr_fnmatch(GIT_PATH, info.path, 0) == APR_SUCCESS) {
	...
	} else if (apr_fnmatch(DEPEND_PATH, info.path, 0) == APR_SUCCESS) {
		...
		return 0;
	} else if (apr_fnmatch(TAR_GZ_PATH, info.path, 0) == APR_SUCCESS) {
		if (info.scheme) {
		...
		rc = Shell_exec(TAR_SH, "FILE", TAR_GZ_SRC);
		check(rc == 0, "Failed to untar %s", TAR_GZ_SRC);
	} else if (apr_fnmatch(TAR_BZ2_PATH, info.path, 0) == APR_SUCCESS) {
		if (info.scheme) {
			rc = Shell_exec(CURL_SH, "URL", url, "TARGET", TAR_BZ2_SRC, NULL);
			check(rc == 0, "Curl failed.");
		}

		apr_status_t rc = apr_dir_make(BUILD_DIR,
				APR_UREAD | APR_UWRITE | APR_UEXECUTE, p);
						...
						const char *make_opts, const char *install_opts)
{
	...
	check(access(BUILD_DIR, X_OK | R_OK | W_OK) == 0,
		"Build directory doesn't exists: %s", BUILD_DIR);

	// actually do an install
	if (access(CONFIG_SCRIPT, X_OK) == 0) {
		log_info("Has a configure script, runing it.");
...
{
	int rc = 0;
	check(Shell_exec(CLEANUP_SH) == 0, "Failed to cleanup before building.");
	...
	if (rc == 1) {
		...
		// no install needed
		log_info("Depends successfullly installed: %s", url);
	} else {
		// had an error
